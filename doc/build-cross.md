Cross-compiliation of Dunduck Core
===============================

Dunduck Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Dunduck.
...
When building Dunduck Core, use
...
Acquire the source in the usual way:
    ...
    git clone https://github.com/Raptor3um/dunduck
    cd dunduck
...
Note that for WSL the Dunduck Core source path MUST be somewhere in the default mount file system, for
example /usr/src/dunduck, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Dunduck Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Dunduck Core source code).
