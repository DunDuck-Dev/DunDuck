...
- [Development guidelines](#development-guidelines)
    - [General Dunduck Core](#general-dunduck-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Dunduck Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Raptor3um/dunduck/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/dunduckd -printtoconsole
```
...
Dunduck Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Dunduck Core, where everyone uses
...
pay attention to for reviewers of Dunduck Core code.

General Dunduck Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Dunduck Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Dunduck Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `dunduck-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `dunduck-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Dunduck Core, the wallet was always
