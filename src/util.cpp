...
const char * const BITCOIN_CONF_FILENAME = "dunduck.conf";
const char * const BITCOIN_PID_FILENAME = "dunduckd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "dunduckd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Dunduck
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Dunduck
    // Mac: ~/Library/Application Support/Dunduck
    // Unix: ~/.dunduck
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Dunduck";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Dunduck";
#else
    // Unix
    return pathRet / ".dunduck";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty dunduck.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Dunduck Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Dunduck Core developers";
    }
